class Admin::OrdersController < Admin::BaseController
  ...
  def index
    params[:status] ||= 'paid'
    @orders = current_admin.show_orders.search_conn(params).order('orders.updated_at desc').page(params[:page]).per(20)
  end
  ...
  private
  def set_order
    @order = current_admin.show_orders.find_by id: params[:id]
